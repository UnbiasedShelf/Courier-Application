...
import by.bstu.vs.stpms.courier_application.ui.courier.model.AvailableOrderViewModel
...
fun AvailableOrdersScreen(
    ...
    viewModel: AvailableOrderViewModel = viewModel()
) {
    LaunchedEffect(true) { viewModel.getAvailableOrders() }
    val scaffoldState = rememberScaffoldState()
    Scaffold(scaffoldState = scaffoldState) {
        viewModel.orders?.let {
            SwipeRefresh(
                state = rememberSwipeRefreshState(viewModel.isRefreshing),
                onRefresh = { viewModel.getAvailableOrders() }
            ) {
                if (it.isSuccessful) {
